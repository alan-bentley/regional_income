...
leed_ta <- read.csv("data/LEED_table37.csv", na.strings=c("","NA"))[,c(1:2,7)] %>% 
  fill(Quarter) %>%
...
leed_reg <- read.csv("data/LEED_table5.csv", na.strings=c("","NA"))[,c(1:2,7)] %>% 
  fill(Quarter) %>%
  rename(area=Region,
         med_inc=Median.earnings.of.full.quarters.jobs) %>% 
  filter(med_inc!="..") %>% 
...
bus_emp <- read.csv(unz("bus_emp_mar22.zip", "machine-readable-business-employment-data-mar-2022-quarter.csv"))
...
HLFS_NZ <- read.csv("data/HLFS_NZ.csv") %>% 
...
HLFS <- read.csv("data/HLFS_med_income.csv", na.strings=c("","NA")) %>% 
  fill(Year) %>% 
